...
XPT2046::XPT2046(int8_t CS, int8_t IRQ, SPIClass* spi) {
    _pinCs = CS;
    _pinIrq = IRQ;
    _spiSettings = SPISettings(2500000, MSBFIRST, SPI_MODE0);
    _spi = (spi != NULL) ? spi : &SPI;
    _maxValue = 0x0fff;
    _width = 0;
    _height = 0;
    _rotation = 0;
...
void XPT2046::read(uint16_t * oX, uint16_t * oY, uint16_t * oZ) {
    ...
    uint32_t widthMax = (_width > 0) ? (_width - 1) : 0;
    uint32_t heightMax = (_height > 0) ? (_height - 1) : 0;

    if(cX <= _minX) {
        cX = 0;
    } else  if(cX >= _maxX || _maxX <= _minX) {
        cX = widthMax;
    } else {
        cX = (cX - _minX) * widthMax / (_maxX - _minX);
    }

    if(cY <= _minY) {
        cY = 0;
    } else if(cY >= _maxY || _maxY <= _minY) {
        cY = heightMax;
    } else {
        cY = (cY - _minY) * heightMax / (_maxY - _minY);
    }
