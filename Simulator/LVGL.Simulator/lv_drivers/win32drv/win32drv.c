    ...
    HICON icon_handle)
{
    ...
    ShowWindowAsync(g_window_handle, show_window_mode);
    ...
    LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        HDC WindowDC = BeginPaint(hWnd, &PaintStruct);
        if (WindowDC && g_buffer_dc_handle && g_display)
        {
            StretchBlt(
                WindowDC,
                0,
                ...
                0,
                0,
                g_display->driver->hor_res,
                g_display->driver->ver_res,
                SRCCOPY);
        }
