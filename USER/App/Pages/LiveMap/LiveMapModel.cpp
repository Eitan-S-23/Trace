...
bool LiveMapModel::GetNavigationInfo(DataProc::Navigation_Info_t* info)
{
    memset(info, 0, sizeof(DataProc::Navigation_Info_t));
    if(account->Pull("Navigation", info, sizeof(DataProc::Navigation_Info_t)) != Account::RES_OK)
    {
        return false;
    }
    return true;
