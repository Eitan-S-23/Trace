...
static bool onLoad(Account* account)
{
    ...
    {
#if CONFIG_LIVE_MAP_SNAPSHOT_ENABLE && (CONFIG_LIVE_MAP_ZOOM_EXTRA_LEVELS > 0)
        /* 高等级放大显示:显示级别允许超过 SD 卡实际数据 EXTRA 级,
         * 超出部分由快照渲染用最高数据级瓦片放大补足(存储零增长)。
         * 扩展级采用 √2/级的平缓阶梯(每 +1 级面积减半),
         * MapConv 依 levelExtra 区分数据级与扩展显示级。 */
        MapConv::SetLevelRange(levelMin, levelMax + CONFIG_LIVE_MAP_ZOOM_EXTRA_LEVELS);
        MapConv::SetExtraDisplayLevels(CONFIG_LIVE_MAP_ZOOM_EXTRA_LEVELS);
#else
        MapConv::SetLevelRange(levelMin, levelMax);
        MapConv::SetExtraDisplayLevels(0);
#endif
    }
    else
    {
