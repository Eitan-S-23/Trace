...
static void * fs_open (lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    if(!file_p->open(fs_normalize_path(path), oflag))
    {
        delete file_p;
        file_p = NULL;
    }
...
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
    ...
    if(!dir_p->open(fs_normalize_path(path)))
    {
        delete dir_p;
        dir_p = NULL;
    }
...
static lv_fs_res_t fs_dir_read (lv_fs_drv_t * drv, void * dir_p, char *fn)
{
    ...
    do
    {
        if(file.openNext(SD_DIR(dir_p), O_RDONLY))
        {
            ...
            if(file.isDir())
            {
                snprintf(fn, LV_FS_MAX_FN_LENGTH, "/%s", name);
            }
            else
            {
                snprintf(fn, LV_FS_MAX_FN_LENGTH, "%s", name);
...
static lv_fs_res_t fs_dir_close (lv_fs_drv_t * drv, void * dir_p)
{
    lv_fs_res_t res = SD_DIR(dir_p)->close() ? LV_FS_RES_OK : LV_FS_RES_FS_ERR;
    delete SD_DIR(dir_p);
    return res;
}
