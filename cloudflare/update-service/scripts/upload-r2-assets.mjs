...
for (const asset of metadata.assets) {
  ...
  const localInfo = await fileInfo(assetPath);
  if (localInfo.sizeBytes !== asset.sizeBytes) {
    fail(`Local size mismatch for ${asset.fileName}`);
  }
  if (localInfo.sha256 !== asset.sha256) {
    fail(`Local sha256 mismatch for ${asset.fileName}`);
  }

  const r2Key = r2KeyForAsset(metadata, asset);
  if (dryRun) {
    process.stdout.write(`[dry-run] ${asset.fileName} -> r2://${bucket}/${r2Key}\n`);
  } else {
    await wranglerWithRetry(
      [
        ...
        "--content-disposition",
        `attachment; filename="${contentDispositionFileName(asset.fileName)}"`,
        "--force"
      ],
    ...
    if (!skipReadback) {
      const tempDir = await mkdtemp(path.join(tmpdir(), "trace-r2-readback-"));
      const readbackPath = path.join(tempDir, asset.fileName);
      try {
        await wranglerWithRetry(
          ["r2", "object", "get", `${bucket}/${r2Key}`, "--file", readbackPath, "--remote"],
          `read back ${asset.fileName}`
        );
        const readbackInfo = await fileInfo(readbackPath);
        if (readbackInfo.sizeBytes !== asset.sizeBytes) {
          fail(`R2 read-back size mismatch for ${asset.fileName}`);
        }
        if (readbackInfo.sha256 !== asset.sha256) {
          fail(`R2 read-back sha256 mismatch for ${asset.fileName}`);
        }
      } finally {
  ...
  asset.r2Key = r2Key;
  asset.r2Verified = true;
}

await writeFile(outputPath, `${JSON.stringify(metadata, null, 2)}\n`, "utf8");
process.stdout.write(`Wrote R2-verified metadata to ${outputPath}\n`);
...
function validateMetadata(value) {
  ...
  if (value.platform !== "android") fail("only android release metadata is supported by this script");
  ...
  if (!Array.isArray(value.assets) || value.assets.length === 0) {
    fail("metadata.assets must be a non-empty array");
  }
}

function r2KeyForAsset(metadata, asset) {
  assertSafeFileName(asset.fileName);
  const releasePrefix = `${metadata.appId}/releases/${metadata.versionCode}-${metadata.releaseTag}`;
...
function contentTypeForAsset(asset) {
  if (asset.assetType === "apk") return "application/vnd.android.package-archive";
  if (asset.assetType === "manifest") return "application/json; charset=utf-8";
  if (asset.assetType === "windows_zip") return "application/zip";
