...
VCDIFF patches generated by xdelta3 must be stripped of the top-level `VCD_APPHEADER` application header before upload. The Flutter `vcdiff_decoder` package rejects files with that header (`Application headers (VCD_APPHEADER) are not supported`). `create_vcdiff_patch.py` owns this compatibility normalization and must keep asserting that generated patches do not contain `VCD_APPHEADER`.
